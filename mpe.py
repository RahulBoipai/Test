...
class MADDPG:
    def __init__(self, actor_dims, critic_dims, n_agents, n_actions,
                 ...
                 gamm=0.99, tau=0.01, chkpt_dir='tmp/maddpg'):
        self.agents=[]
        self.n_agents=n_agents
        ...
        for agent_idx in range(self.n_agents):
            self.agents.append(Agent(actor_dims[agent_idx], critic_dims,
                                     n_actions, n_agents, agent_idx, alpha=alpha, beta=beta, chkpt_dir=chkpt_dir))
    ...
    def learn(self,memory):
        ...
        action=T.tensor(actions, dtype=T.float).to(device)
        ...
        for agent_idx, agent in enumerate(self.agents):
            ...
            critic_value = agent.critc.forward(states, old_actions).flatten()

            target=rewards[:,agent_idx]+agent.gamma*critic_value_
            critc_loss=F.mse_loss(target, critic_value)

            agent.critic.optimizer.zero_grad()
            critc_loss.backward(retain_graph=True)
            agent.critic.optimizer.step()

            actor_loss = agent.critc.forward(states, mu).flatten()
            actor_loss = T.mean(actor_loss)
            agent.actor.optimizer.zero_grad()
            actor_loss.backward(retain_graph=True)
            agent.actor.optimizer.step()

            agent.update_network_parametes()
...
if __name__ == '__main__':
    ...
    critc_dims = sum(actor_dims)
    
    n_actions = env.action_space(env.agents[0]).n

    maddpg_agents = MADDPG(actor_dims, critc_dims, n_agents, n_actions,
                           fc1=64, fc2=64, alpha=0.01, beta=0.01, scenario=scenario,
                           chkpt_dir='tmp/maddpg/')
    memory=MA_ReplayBuffer(1000000, critc_dims, actor_dims, n_actions, n_agents, batch_size=1024)
    ...
    for i in range(N_GAMES):
        ...
        while not any(done):
            ...
            take_action(env,actions)
            obs_=get_env_obs(env)
            reward=get_env_rwd(env)
